...
int __bst__postorder(BSTNode *root) {
    if (root) {
...
void kalobsatu(BST *bst, int n, int x){
	int j, rem = 0;
	for(j = 0; j < n; j++){
		if(apost[j] == x){
			bst_remove(bst, x);
	...
	if(rem) printf("%d sudah terhapus\n", x);
	//else printf("%d tidak ada\n", x);
}

void kalobdua(BST *bst, int n, int x){
	int i, rem = 1;
	for(i = 0; i < bst->_size; i++){
		if(apost[i] == x){
			rem = 0;
			printf("%d ada di index ke-%d\n", x, i + 1);
		}
	}
	if(rem) printf("%d tidak ada\n", x);
}
